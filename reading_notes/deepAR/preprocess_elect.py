# the code is taken from https://github.com/zhykoties/TimeSeries
# most comments are put in by Yao for helping understanding 
...
def prep_data(data, covariates, data_start, train = True):
    ...
    # I don't completely understand the calculation here about input, window, stride
    ...
    # each series has its own number of windows
    # sum all windows together and make a train set. this explains why the training dataset is large
    total_windows = np.sum(windows_per_series)
   
    # the calculation of x_input seems complicated
    # overall, the first dimension is the total number of windows
    # the second window_size, which is the same for every window, corresponding to how many rows are available in making forecasting
    # the last dimension is 1 + num_covariates + 1
    # the first 1 is for target data, the last 1 for the index of the series, i.e. investment_id
    ...
    # count is iterating over range(total_windows)
    count = 0
    if not train:
    ...
    for series in trange(num_series):
        ...
        for i in range(windows_per_series[series]):
            ...
            # the first entry is target
            x_input[count, 1:, 0] = data[window_start:window_end-1, series]
            x_input[count, :, 1:1+num_covariates] = covariates[window_start:window_end, :]
            
            # the last entry is index of series, notice that series is index
...
def gen_covariates(times, num_covariates):
    # generate relative temporal location as covariates
...
if __name__ == '__main__':
    ...
    # original data's period is 15min, resample to 1h
    data_frame = data_frame.resample('1H',label = 'left',closed = 'right').sum()[train_start:test_end]
    data_frame.fillna(0, inplace=True)
    
    # the original dataset has no covariates
    # each entry in data_frame is a target value
    # each col corresponds to a series (i.e. investment id)
    # each row corresponds to a time point
    
    
    # relative temporal location is generated as covariates
    ...
    # different series have different start time, but have the same period once it starts
    ...
    # the key task is to understand prep_data function, which is confusingly written
