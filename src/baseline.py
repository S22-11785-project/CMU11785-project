...
################################################################################
def main(args):
    print('Start main loop'.center(50, "-"))
    ...
    train_dataset = datasets.UbiquantDatasetOriginal(df_train, invst_ids)
    val_dataset = datasets.UbiquantDatasetOriginal(df_eval, invst_ids)
    train_loader = DataLoader(train_dataset, batch_size=batch_size)
    val_loader = DataLoader(val_dataset, batch_size=batch_size)

    print('Create model'.center(50, "-"))
    ...
    for epoch in range(args['starting_epoch'], args['epochs']+args['starting_epoch']):
        ...
        print(f"[{time.asctime()}] - Epoch {epoch}/{args['epochs']}: Train Loss {train_loss}, Val Loss {val_loss}, Learning Rate {lr_o}, finished in {e_time} seconds.")
        if 'wb_project' in args: 
            wandb.log({"Train Loss": train_loss, "Val Loss": val_loss, "Time Used (s)": e_time, "lr": lr_o})

    print('Training completes'.center(50, "-"))
...
if __name__ == '__main__':
    args = {
        'subset': 0.1, # portion of data to use (subset for developing)
        'batch_size': 256,
        'starting_epoch': 1,
        'epochs': 15,
        'lr': 1e-3,
