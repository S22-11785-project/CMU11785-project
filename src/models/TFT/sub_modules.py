...
class InterpretableMultiHeadAttention(nn.Module):
    ...
    def forward(self, q, k, v, mask=None) -> Tuple[torch.Tensor, torch.Tensor]:
        ...
        for i in range(self.n_head):
            ...
            # ---------------------------------- Change ---------------------------------- #
            head, attn, score = self.attention(qs, ks, vs, mask) # Added score
            # ---------------------------------- Change ---------------------------------- #
        ...
        print("==>> scor.shape: ", scor.shape)
        print("==>> torch.mean(head, dim=2).shape: ", torch.mean(head, dim=2).shape)
        print("==>> scor.permute(0, 2, 1).shape: ", scor.permute(0, 2, 1).shape)
